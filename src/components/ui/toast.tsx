  ...
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
        ...
        success: 'success group border-green-500 bg-green-50 text-green-900 dark:bg-green-900/20 dark:text-green-400',
        warning: 'warning group border-yellow-500 bg-yellow-50 text-yellow-900 dark:bg-yellow-900/20 dark:text-yellow-400',
        info: 'info group border-blue-500 bg-blue-50 text-blue-900 dark:bg-blue-900/20 dark:text-blue-400',
      },
    },
    defaultVariants: {
      variant: 'default',
...
type ToastProps = React.ComponentPropsWithoutRef<typeof ToastPrimitive.Root> &
  VariantProps<typeof toastVariants> & {
    ...
    description?: string;
    /**
     * Optional action to show in the toast
     */
  ...
  ({ className, variant, title, description, action, icon, ...props }, ref) => {
    return (
      ...
      >
        ...
        {action}
        <ToastClose className="absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50">
          <X className="h-4 w-4" />
        </ToastClose>
      </ToastPrimitive.Root>
    );
